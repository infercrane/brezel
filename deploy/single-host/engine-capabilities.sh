...
check_source() {
  ...
  require_literal "$source_root/embed/compose/scripts/node/build-base-template.mjs" \
    "BASE_TEMPLATE_SWAP_MB" "the bounded base-template swap reserve"
  require_literal "$source_root/packages/orchestrator/pkg/sandbox/reclaim.go" \
    'swapfile=/var/lib/brezel/swapfile' "the Brezel-owned guest swapfile path"
  require_literal "$source_root/packages/orchestrator/pkg/sandbox/reclaim.go" \
    '/sbin/swapon "$swapfile"' "the post-resume guest swap activation"
  require_literal "$source_root/packages/orchestrator/pkg/sandbox/sandbox.go" \
    'failed to restore sandbox guest swap' "fail-closed snapshot-resume swap activation"
  require_literal "$source_root/packages/orchestrator/pkg/sandbox/reboot.go" \
    'restore guest swap after reboot' "fail-closed reboot swap activation"
  require_literal "$source_root/packages/orchestrator/pkg/sandbox/guest_swap_test.go" \
    'TestEnsureGuestSwapWithRunnerFailsClosed' "the guest swap activation regression test"
