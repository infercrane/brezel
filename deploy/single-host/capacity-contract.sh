...
[ "$MAX_ACTIVE_TOTAL" -le "$NETWORK_NEW_SLOTS" ] || \
  fail "the active limit cannot exceed the $NETWORK_NEW_SLOTS-slot new-sandbox network pool"
[ "$MAX_ACTIVE_TOTAL" -le "$NETWORK_REUSED_SLOTS" ] || \
  fail "the active limit cannot exceed the $NETWORK_REUSED_SLOTS-slot reused-sandbox network pool"
required_nbd_slots=$((MAX_ACTIVE_TOTAL + HEADROOM_SANDBOXES))
...
required_hugepages=$(( ((MAX_ACTIVE_TOTAL + HEADROOM_SANDBOXES) * GUEST_MEMORY_MIB + 1) / 2 ))
