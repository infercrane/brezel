...
ENGINE_GUEST_SWAP_PATCH="$REPO_DIR/third_party/e2b-runtime/patches/0024-activate-guest-swap-after-resume.patch"
...
export BREZEL_GUEST_VCPUS BREZEL_GUEST_MEMORY_MIB BREZEL_GUEST_SWAP_MIB BREZEL_GUEST_MIN_FREE_DISK_MIB BREZEL_GUEST_MAX_FREE_DISK_MIB
...
ENGINE_GUEST_SWAP_PATCH_SHA256=$(read_lock orchestrator_guest_swap_patch_sha256)
...
if [ "$(sha256sum "$ENGINE_GUEST_SWAP_PATCH" | awk '{print $1}')" != "$ENGINE_GUEST_SWAP_PATCH_SHA256" ]; then
  echo "engine guest-swap patch verification failed" >&2
  exit 1
fi
...
patch --batch --forward --fuzz=0 -d "$ENGINE_BUILD_DIR" -p1 < "$ENGINE_GUEST_SWAP_PATCH"
...
"$ARTIFACT_SUPPLY_CHAIN" manifest "$INSTALL_DIR/distribution.manifest" "$LOCK_FILE" "$ENGINE_IMAGE_LOCK" "$ENGINE_ARTIFACT_LOCK" / \
  ...
  "$ENGINE_REFLINK_SPARSE_MATERIALIZATION_PATCH_SHA256" "$ENGINE_GUEST_SWAP_PATCH_SHA256"
